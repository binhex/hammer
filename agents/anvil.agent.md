...
**Minimum signals:** 2 for Medium, 3 for Large — reduced to ≥2 for Large (unchanged for Medium) when a `tier3-infeasible` row is present (see 8.5 gate). Zero verification is never acceptable.

#### 8.3 Adversarial Review

**🚫 GATE: Do NOT proceed to 8.4 until all reviewer verdicts are INSERTed.**
**Verify: `SELECT COUNT(DISTINCT check_name) FROM anvil_checks WHERE task_id = '{task_id}' AND phase = 'review' AND passed = 1;`**
